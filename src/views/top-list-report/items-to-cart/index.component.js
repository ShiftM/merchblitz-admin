...
export default {
  ...
  async mounted() {
    await this.itemAddedList(this.filter);
    await this.getInterestOption(this.filter_options);
    await this.getCityOption(this.filter_options);
  },
  methods: {
    async search(data) {console.log(this.filter);
      if (this.validAge && this.validDate) {
        this.filter = Object.assign(this.filter, data);
        if (this.filter.type === 1) {
          await this.itemAddedList(this.filter);
        } else {
          await this.itemRemovedList(this.filter);
        }
      }
    },
    async itemAddedList(data) {
      this.data = await topListReportService.topItemsAddedToCart(data);
    },
    async itemRemovedList(data) {
      this.data = await topListReportService.topItemsRemovedToCart(data);
    },
    async paginate(page) {
