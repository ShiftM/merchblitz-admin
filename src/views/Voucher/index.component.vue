    ...
    <div class="nav-tabs custom-tabs">
      <div class="nav-item primary">
        <button type="button" @click="search({status_option_id: 1, page: 1}), toggleTab()" class="nav-link"
                :class="{'active': filter.status_option_id == 1 }">Voucher
        </button>
      </div>
        ...
        <div class="table-responsive">
          <table class="table table-bordered">
            ...
            <tr>
              ...
              <th width="10%">Code</th>
              <th width="10%">Type</th>
              <th width="15%">Number of Uses</th>
              <th width="15%">Use per Customer</th>
            ...
            <tr v-for="(value,key) in data.data">
              <td>{{value.name}}</td>
              <td>{{value.code}}</td>
              <td>{{value.voucher_type.name}}</td>
              <td>{{value.number_of_use}}</td>
              <td>{{value.use_per_customer}}</td>
              <td>{{value.expiry_date}} {{value.expiry_time}}</td>
              <td v-if="filter.status_option_id == 1">

                <router-link tag="a" class="text-primary"
                             :to="{name: 'voucher-edit', params: {id: value.id}}">Edit
                </router-link>
