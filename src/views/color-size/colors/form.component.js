...
export default {
    ...
    methods: {
        ...
        async store() {
            const cb = async () => {
                const response = await colorOptionService.store(this.data);
                if (response.code == 422) {
                    ...
                    alertService.successWithMessage('Success','Color created successfully.');
                    this.$router.go(-1);
                }
            }
        ...
        async update() {
            const cb = async () => {
                const response = await colorOptionService.update(this.$route.params.id, this.data);
                if (response.code == 422) {
                    this.errors = response.errors;
                }else{
                    this.data = {
        ...
        async show(id) {
            this.data = await colorOptionService.show(id);
        }
    }
