    ...
    <div class="nav-tabs custom-tabs">
      <div class="nav-item primary">
        <button type="button" @click="search({status_option_id: 1, page: 1}), toggleTab()" class="nav-link "
          :class="{'active': filter.status_option_id == 1 }">City Options
        </button>
      </div>
      ...
      <div class="col-12 mt-3">
        <router-link :to="{name: 'city-create' }">
          <button type="button" class="btn btn-success mb-2">
            <i class="fa fa-plus"></i>&nbsp; Add City Option
          </button>
        </router-link>
          ...
          <table class="table table-bordered">
            ...
            <tbody>
              <tr v-for="(value,key) in data.data">
                <td>{{value.name}}</td>
                <td>{{value.province_option.name}}</td>
                ...
                <td v-if="filter.status_option_id == 1">
                  <router-link tag="a" class="text-primary"
                    :to="{name: 'city-edit', params: {id: value.id}}">Edit
                  </router-link>
