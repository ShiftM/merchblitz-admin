        ...
        <div class="nav-tabs custom-tabs">
            <div class="nav-item primary">
                <button type="button" @click="search({status_option_id: 1, page: 1,  has_featured: false}), toggleTab()"
                        class="nav-link "
                        :class="{'active': !filter.has_featured && filter.status_option_id == 1 }">Administrator
                </button>
            </div>
        </div>
        <div class="tab-content">
            <div class="col-12 mt-3">
                <div class="input-group">
                    <input v-model="filter.name" class="form-control" id="item" name="items" size="16" placeholder="Administrator name" list="items"
                        ...
                        <tbody>
                            <template v-for="(value,key) in data.data">
                                <tr>
                                    ...
                                    <td>
                                        <template>
                                            <router-link :to="{ name: 'administrator-edit', params: { id:value.id }}" tag="a">
                                                Change Password
                                            </router-link>
