  ...
  <div class="card-body table-responsive">
    <div class="nav-tabs custom-tabs">
      <div class="nav-item primary">
        <button type="button" @click="search({status_option_id: 1, page: 1}), toggleTab()" class="nav-link "
          :class="{'active': filter.status_option_id == 1 }">Province Options
        </button>
      </div>
    ...
    <div class="tab-content">
      <div class="col-12 mt-3">
        <router-link :to="{name: 'province-create' }">
          <button type="button" class="btn btn-success mb-2">
            <i class="fa fa-plus"></i>&nbsp; Add Province Option
          </button>
        </router-link>
        <div class="table-responsive">
          <table class="table table-bordered">
            ...
            <tbody>
              <tr v-for="(value,key) in data.data">
                <td>{{value.name}}</td>
                <td>{{value.region_option.name}}</td>
                <td>{{value.country_option.name}}</td>
                <td v-if="filter.status_option_id == 1">
                  <router-link tag="a" class="text-primary"
                    :to="{name: 'province-edit', params: {id: value.id}}">Edit
                  </router-link>
          ...
          <p v-if="noData" class="text-center">
